...
class Level_Structure():
            ...
            for i in range(size_x):
                if map[i][j] == WALL: 
                    if not Level_Structure.Wall_Checker(map, i, j, size_x, size_y, tilemap.tilemap):
                        tilemap.tilemap[str(i) + ';' + str(j)] = {'type': 'wall_bottom', 'variant': 0, 'pos': (i, j), 'active': 0, 'light': 0}
    ...
    def Wall_Checker(map, i, j, size_x, size_y, tilemap):
        ...
        if i <= 1:
            tilemap[str(i) + ';' + str(j)] = {'type': 'wall_left', 'variant': random_variant, 'pos': (i, j), 'active': 0, 'light': 0}
            return True

        elif i >= size_x - 2:
            tilemap[str(i) + ';' + str(j)] = {'type': 'wall_right', 'variant': random_variant, 'pos': (i, j), 'active': 0, 'light': 0}
            return True
        
        elif j <= 1:
            tilemap[str(i) + ';' + str(j)] = {'type': 'wall_top', 'variant': random_variant, 'pos': (i, j), 'active': 0, 'light': 0}
            return True
        ...
        if map[i + 1][j] != WALL and map[i - 1][j] != WALL:
            tilemap[str(i) + ';' + str(j)] = {'type': 'wall_middle', 'variant': random_variant, 'pos': (i, j), 'active': 0, 'light': 0}
            return True
    ...
    def Corner_Handling(map, i, j, random_variant, tilemap) -> bool:
        ...
        both_sides = 2
        if map[i + 1][j] != WALL and map[i - 1][j] != WALL:
            tilemap[str(i) + ';' + str(j)] = {'type': 'wall_bottom_corner', 'variant': both_sides, 'pos': (i, j), 'active': 0, 'light': 0}

        elif map[i + 1][j] != WALL:
            tilemap[str(i) + ';' + str(j)] = {'type': 'wall_bottom_corner', 'variant': right_side, 'pos': (i, j), 'active': 0, 'light': 0}

        elif map[i - 1][j] != WALL:
            tilemap[str(i) + ';' + str(j)] = {'type': 'wall_bottom_corner', 'variant': left_side, 'pos': (i, j), 'active': 0, 'light': 0}

        else:
            tilemap[str(i) + ';' + str(j)] = {'type': 'wall_bottom', 'variant': random_variant, 'pos': (i, j), 'active': 0, 'light': 0}
        return True